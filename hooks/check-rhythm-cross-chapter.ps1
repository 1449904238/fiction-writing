#!/usr/bin/env pwsh
# check-rhythm-cross-chapter.ps1 — 跨章节奏检测hook（V1.0新增）
...
# 检查是否为5的倍数
if ($TotalChapters % 5 -ne 0) {
    Write-Host "[check-rhythm] 章节数 $TotalChapters 非5的倍数，跳过（每5章触发一次）" -ForegroundColor Gray
    exit 0
}
